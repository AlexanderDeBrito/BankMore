            ...
            .AddConsumer(consumer =>
                consumer
                    ...
                    .AddMiddlewares(middlewares =>
                        middlewares.AddTypedHandlers(h =>
                            h.AddHandler<TransferConsumer>()
                        )
                    )
            )
            .AddProducer("tariff-producer", producer =>
                producer.DefaultTopic(builder.Configuration["Kafka:TariffTopic"])
            )
    )
);
...
builder.Services.AddScoped<IMessageHandler, TransferConsumer>();
